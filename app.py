...
def login():
    if request.method == 'POST':
        ...
        # If you expect only one result, this is sufficient

        # Always make sure to fetch all if necessary or call nextset() if there are multiple sets
        # cursor.fetchall()  # Uncomment if you want to ensure all results are fetched
        ...
        if user and check_password_hash(user['password'], password):
            ...
            if user['role'] == 'student':
                return redirect(url_for('index'))  # Adjust the endpoint as necessary
            elif user['role'] == 'head':
                return redirect(url_for('eventhead'))  # Adjust the endpoint as necessary
...
@app.route('/eventhead')
def eventhead():
    return render_template('eventhead.html') 
